...
void print_matInfo(string name, Mat img)
{
	...
	if (depth == CV_8U)
		...
		str = "CV_64F";
	cout << name;
	cout << format(": depth(%d) channels(%d) -> ?ë£Œí˜•: ", depth, img.channels());
	cout << str << "C" << img.channels() << endl;
}

int main()
{
	string filename1 = "images/read_16.tif"; // 16ë¹„íŠ¸ ?˜?ƒ
	string filename2 = "images/read_32.tif"; // 32ë¹„íŠ¸ ?˜?ƒ
	...
	cout << "16/32ë¹„íŠ¸ ?˜?ƒ ?–‰? ¬ ì¢Œí‘œ (100,100) ?™”?†Œê°? " << endl;
